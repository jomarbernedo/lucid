...
contract eTaya {
    ...
    constructor() public {
        owner = msg.sender;
        verifier = msg.sender;
        WinnersDB[20200101] = winners([1,2,3,4,5,6],8648391,true);
        winningDates.push(20200101);
    ...
    // posts and verifies userID
    bool userExisting;
    mapping (bytes32 => bool) public UserDB; 
    event UserExisting(bool userExisting);

    function checkUser(bytes32 userID) external view returns(bool) {
        return UserDB[userID];
    }

    function newUser(bytes32 userID) external returns(bool) {
        //check if entry already in BetsDB
        require(!UserDB[userID], "User already existing.");

        //post the user 
        UserDB[userID] = true;

        //emit event true
        emit UserExisting(true);
...
//    mapping (bytes32 => bet) public BetsDB;
    ...
    function getBetStatus(bytes32 _txnHash) public view returns(bool, bool) {
...
//        require(BetsDB[_txnHash].isExisting, "Bet is not existing.");
    ...
    mapping (uint => winners) public WinnersDB;

    // list of all dates with winning numbers posted
    uint[] public winningDates;

    function getLength() public view returns(uint, uint[] memory) {
        return(winningDates.length,
                winningDates);
    }
    ...
    function postWinners(uint _drawdate, uint8[6] calldata _winners, uint _prize) external isVerifier {
        ...
        // add date to winningDates array
        winningDates.push(_drawdate);
    ...
    function getWinners(uint _drawdate) public view returns(uint, uint8[6] memory, uint) {
